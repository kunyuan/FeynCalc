...
mu = np.loadtxt(Para.DataFolder+"/dMu.data")
dMu2, dMu3, dMu4 = mu[0, :]
...
if Quan == 'Polar':
    for o in range(1, Para.Order+1):
        ...
        print ("Order {0}: {1:12.8f} +-{2:12.8f}, Accu: {3:12.8f} +-{4:12.8f}".format(
            o, Each[o][0, 0], Each[o][1, 0]*1.0, Accu[o][0, 0], Accu[o][1, 0]*1.0))
    plt.plot(KGrid/Para.kF, -BubbleQ, lw=2, color="black", label="ideal")
    ax.set_ylabel("$-P(\omega=0, q)/N_F$", size=size)
elif Quan == 'Chi':
    for o in range(1, Para.Order+1):
        Chi = -2.0*Accu[o][0, :]/(1+8.0*np.pi/KGrid**2*Accu[o][0, :])
        plt.errorbar(KGrid/Para.kF, Chi, yerr=0.0, fmt='o-', ms=4, capthick=1, capsize=4,
                     color=ColorList[o], label="Order {0}".format(o))
    plt.plot(KGrid/Para.kF, 2*BubbleQ, lw=2, color='orange', label="ideal")
    Chi_RPA = 2*BubbleQ/(1-8.0*np.pi/KGrid**2*BubbleQ)
    plt.plot(KGrid/Para.kF, Chi_RPA, lw=2, linestyle='--', color='black', label="RPA")
    ax.set_ylabel("$\chi(\omega=0, q)$", size=size)
elif Quan == 'LFC':
    for o in range(1, Para.Order+1):
        ...
        print ("Order {0}: {1:12.8f} +-{2:12.8f}, Accu: {3:12.8f} +-{4:12.8f}".format(
            o, Each[o][0, 0], Each[o][1, 0]*1.0, Accu[o][0, 0], Accu[o][1, 0]*1.0))
    ax.set_ylabel("$G(\omega=0, q)$", size=size)
    ax.set_ylim(0,)
elif Quan == 'Dielec':
    for o in range(1, Para.Order+1):
        epsilon = 1.0/(1+8.0*np.pi/KGrid**2*Accu[o][0, :])
        plt.errorbar(KGrid/Para.kF, epsilon, yerr=0.0, fmt='o-', capthick=1, capsize=4,
                     color=ColorList[o], label="Order {0}".format(o))
    epsilon_RPA = 1.0/(1-8.0*np.pi/KGrid**2*BubbleQ)
    plt.plot(KGrid/Para.kF, epsilon_RPA, lw=2, linestyle='--', color='black', label="RPA")
    ax.set_ylabel("$1/\epsilon(\omega=0, q)$", size=size)
...
plt.savefig(Quan+'_charge_beta{0}_rs{1}_lam{2}.pdf'.format(beta,rs,lam))
plt.show()
