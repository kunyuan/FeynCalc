...
for index, eachline in enumerate(inlist):
    ...
    QGrid = np.linspace(0, 0.1*Para.kF, 5, endpoint=False)
    ...
    for QBin, extQ in enumerate(QGrid):            
        DataDict, Step, Groups, ReWeight, Grids = LoadFile(Para.DataFolder, "pid[0-9]+_q{0}.dat".format(QBin))
        ...
        filename = './Data_Polar/polar_beta{0}_rs{1:3.1f}_lam{2}_o{3}_q{4}.dat1'.format(beta, rs, lam, Para.Order, QBin)
