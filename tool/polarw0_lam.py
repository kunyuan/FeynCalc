...
for index, eachline in enumerate(inlist):
    ...
    for (o, key) in enumerate(sorted(EsDataDict.keys())):
        if key == (0, ):
            continue
        # if o == 1 and Iflq and (beta != 2.0 and beta !=1.0):  #and Para.Order==5 
        #     EsDataDict[key][0] -= BubbleQ[:, 0]
        # if o == 1 and Ifhq and beta in [2.0, 4.0]:
        #     EsDataDict[key][0] -= BubbleQ[:, 0]
        if o == 1:
            EsDataDict[key][0] += BubbleQ[:, 0]
        if key == (1, 0, 1, 2):
    ...
    filename = './Data_Polar/polar_beta{0}_rs{1:3.1f}_lam{2:.1f}_o{3}.dat'.format(beta, rs, lam, Para.Order)
